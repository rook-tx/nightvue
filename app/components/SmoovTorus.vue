...
export default {
	...
	methods: {
		init() {
			...
			this.glowPass.uniforms.amount.value = 2;
		...
		update() {
			...
			let time = performance.now(),
				scaledTime = time * (this.bpm / 30000),
				deepScale = time * (this.bpm / 120000);

			for (let i = 0; i < this.orbs.length; i++) {
				let step = Math.PI * 5 * i / this.orbCount * (8.47 / 55 * this.orbCount),
					theta = scaledTime + step,
					orbRadius = this.radius / 10 +
						this.radius * (Math.cos(deepScale + step) + 1),
					orbDepth = this.radius * 1.5 * Math.sin(deepScale + step);
			...
			this.camera.position.z += 0.02 * (this.deltaZ - this.camera.position.z);

			this.camera.lookAt(this.scene.position);

			this.camera.rotation.z += deepScale / 6;
		...
		momo(e) {
			...
			this.deltaZ = this.radius;
			this.deltaX = this.mouseX / (this.width / 2) * (this.radius / 8);
			this.deltaY = this.mouseY / -(this.height / 2) * (this.radius / 8);
		}
	},
