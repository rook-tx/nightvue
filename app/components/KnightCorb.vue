...
export default {
	...
	methods: {
		...
		update() {
			...
			let time = performance.now(),
				scaledTime = time * (this.bpm / 80000);

			for (let i = 0; i < this.orbs.length; i++) {
				let step = Math.PI * 2 * i / this.orbCount,
					theta = scaledTime / 2 * step,
					orbRadius = 600 + this.radius / 2 * (Math.cos(scaledTime + step) + 1),
			...
			this.camera.position.x += 0.011 * (this.deltaX - this.camera.position.x);
			this.camera.position.y += 0.011 * (this.deltaY - this.camera.position.y);
			this.camera.position.z += 0.011 * (this.deltaZ - this.camera.position.z);

			this.camera.lookAt(this.scene.position);

			this.camera.rotation.z += scaledTime / 8;

			this.wobblePass.uniforms.time.value = scaledTime / 4;
	...
	mounted() {
		...
		window.addEventListener('blur', this.stop, { passive: true });
		window.addEventListener('focus', this.start, { passive: true });
	},
	...
	beforeDestroy() {
		...
		window.removeEventListener('blur', this.stop, { passive: true });
		window.removeEventListener('focus', this.start, { passive: true });
	}
