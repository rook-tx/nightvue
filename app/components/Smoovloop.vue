...
export default {
	...
	methods: {
		init() {
			...
			this.glowPass.uniforms.amount.value = 5;

			this.wobblePass = new THREE.ShaderPass(THREE.WobbleShader);
			this.wobblePass.uniforms.size.value = 2;
		...
		update() {
			...
			for (let i = 0; i < this.orbs.length; i++) {
				...
				let theta = scaledTime * i / 95;
				// ishift = this.orbCount - i;
				...
				let orbRadius = this.radius * (Math.sin(scaledTime * i / 95 / 16) + 1);
				let orbDepth = this.radius * Math.sin(scaledTime * i / 95 / 16);
		...
		momo(e) {
			...
			this.deltaZ = 0;
			this.deltaX = this.mouseX / (this.width / 2);
			this.deltaY = this.mouseY / (this.height / 2);
		}
	},
